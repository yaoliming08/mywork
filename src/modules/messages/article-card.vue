<template>
    <div class="article-card message-component" :class="clazz">
        <Avatar :message="message"></Avatar>

        <div class="text-content-wrap">
            <div class="text-content">
                <section>
                    <section class="title">文章</section>
                    <section class="date">
                        <p> 标题：{{ message.contentStr?.title }}</p>
                        <p> 医生：{{ message.contentStr?.author }}</p>
                    </section>
...
import { type ArticleMessage } from '@/model/message'
...
const props = defineProps({
    message: { type: Object as PropType<ArticleMessage>, required: true },
})

console.log(props.message, 1777777777777)
const clazz = chatManager.isCurrentUser(props.message.sender?.userId) ? 'right' : 'left'

async function openMedicalRecordDetail() {  //跳转H5文章页面

    let webViewSrc = encodeURIComponent(`/consult/article/${props.message.contentStr.id}`)
    uni.navigateTo({
